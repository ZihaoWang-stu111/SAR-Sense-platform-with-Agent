...
from api.dependencies import get_metrics, get_metrics_repository
...
async def get_metrics_data(user: dict = Depends(get_current_user)):
    ...
    data = await run_in_threadpool(repository.aggregate)
...
async def reset_metrics(admin: dict = Depends(require_admin)):
    ...
    repository = get_metrics_repository()
    metrics = get_metrics()
    await run_in_threadpool(repository.reset, metrics.reset)
