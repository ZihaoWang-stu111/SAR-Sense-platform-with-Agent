...
class ChatAgentDispatchTest(unittest.IsolatedAsyncioTestCase):
    async def test_chat_submits_agent_work_to_shared_executor(self):
        ...
        with (
            ...
            patch.object(chat_runner, "_try_load_memory_context", return_value=""),
        ):
            chat_runner.submit_chat_run(
                messages=[{"role": "user", "content": "测试问题"}],
                memory_user_message="测试问题",
        ...
        self.assertEqual(events[0], ("status", "正在思考..."))
        self.assertIn(("chunk", "测试回答"), events)
