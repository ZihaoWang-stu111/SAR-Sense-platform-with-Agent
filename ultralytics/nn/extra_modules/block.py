...
# -------------------------
# 保留：MeanShift（一般 SAR 不用它，但保留不影响）
# -------------------------
...
# -------------------------
# 删减：ShiftConv2d0（可选）
# 说明：为了兼容你原工程，这里仍保留类定义，但建议不再使用它（ShiftConv2d 默认走 2d1）
# -------------------------
class ShiftConv2d0(nn.Module):
    def __init__(self, inp_channels, out_channels):
        super().__init__()
        # 仍保留：以免你工程里有人调用 low-training-memory 分支
        ...
        mask = nn.Parameter(torch.zeros((out_channels, inp_channels, 3, 3)), requires_grad=False)
    ...
    def forward(self, x):
        return F.conv2d(x, self.w * self.m, self.b, stride=1, padding=1)


# -------------------------
# 核心：ShiftConv2d1（保留你的轻量主干：固定 shift + 1×1）
# 不增加参数，不改接口
# -------------------------
    ...
    def __init__(self, inp_channels, out_channels):
        ...
        with torch.no_grad():
            ...
            if g > 0:
                ...
                self.weight[:, 0, 1, 1] = 1.0

        self.conv1x1 = nn.Conv2d(inp_channels, out_channels, 1)

    def forward(self, x):
        y = F.conv2d(x, self.weight, bias=None, stride=1, padding=1, groups=self.inp_channels)
...
class ShiftConv2d(nn.Module):
    def __init__(self, inp_channels, out_channels, conv_type='fast-training-speed'):
        ...
        # 建议：默认只用 fast-training-speed（ShiftConv2d1），最轻最稳
...
# - log压缩
# - 方向性背景估计（1×k 和 k×1）
# - top-hat lite（max-mean）
# 计算量：两个 1D avgpool + 一个 maxpool（极轻）
# -------------------------
class _SARLiteEnhance(nn.Module):
    def __init__(self, k_bg=9, k_hat=3, eps=1e-6):
        super().__init__()
        self.k_bg = k_bg
        self.k_hat = k_hat
        self.eps = eps
    ...
    def forward(self, x):
        # 1) signed log1p：压缩 SAR 强散射动态范围（对小船更友好）
        xa = torch.abs(x)
        x_log = torch.sign(x) * torch.log1p(xa + self.eps)

        # 2) 方向性背景估计：水平/垂直（CFAR-like）
        pad = self.k_bg // 2
        bg_h = F.avg_pool2d(x_log, kernel_size=(1, self.k_bg), stride=1, padding=(0, pad))
        bg_v = F.avg_pool2d(x_log, kernel_size=(self.k_bg, 1), stride=1, padding=(pad, 0))
        bg = torch.minimum(bg_h, bg_v)  # 更保守的背景（更像 CFAR）

        high = x_log - bg  # 背景抑制

        # 3) top-hat lite：max - mean，突出小亮点/边缘
        p2 = self.k_hat // 2
        loc_max = F.max_pool2d(x_log, kernel_size=self.k_hat, stride=1, padding=p2)
        loc_mean = F.avg_pool2d(x_log, kernel_size=self.k_hat, stride=1, padding=p2)
        hat = loc_max - loc_mean

        # 4) 固定融合（无参数）：保留原信息，避免过强预处理导致掉点
        #   - high 强化目标与背景差异
        #   - hat 强化小亮点（船体强散射）
        return x_log + 0.25 * high + 0.15 * hat


# -------------------------
# LFE：删掉之前不稳定的门控/强归一化，只保留这个“对症+轻量”的增强
# 参数结构不变：训练参数仍只有 conv1x1
# -------------------------
class LFE(nn.Module):
    def __init__(self, inp_channels, out_channels, exp_ratio=4, act_type='relu'):
        ...
        # 新增无参增强模块（不增加可训练参数）
        self.enhance = _SARLiteEnhance(k_bg=9, k_hat=3)
...
class Bottleneck_IDWC(Bottleneck):
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):
        super().__init__(c1, c2, shortcut, g, k, e)
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = InceptionDWConv2d(c_)
        self.cv2 = InceptionDWConv2d(c_)
