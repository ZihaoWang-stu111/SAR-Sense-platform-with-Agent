...
class AutoBackend(nn.Module):
    ...
    def __init__(
        ...
        if not (pt or nn_module):
            raise ValueError("精简检测运行时仅支持本地 .pt 权重")
    ...
    def _model_type(p="path/to/model.pt"):
        """Return the backend flags needed by SAR-Sense's PyTorch-only inference."""
        pt = isinstance(p, str) and Path(p).suffix.lower() == ".pt"
        return [pt] + [False] * 13
