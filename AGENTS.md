...
- **react_agent.py** — `ReactAgent` wrapping LangChain's `create_agent`. Streams via `execute_stream(chat_pack, conversation_id=None, on_step=None)`. Thought-chain steps are pushed via the `on_step` callback (no global state) — callers collect them in request-local lists. `on_step` is optional, so legacy direct calls still work. **There is no global `_thought_chain` dict anymore.** Complex research tasks (cross-paper comparison, multi-source synthesis) are delegated via the `delegate_research` tool to a `sar-researcher` sub-agent; the sub-agent's execution trace bridges back through a request-local `_subagent_on_step` callback (-> `thought_steps`) and RAG sources through a request-local `_subagent_rag_results` collector (-> `rag_results`), while only the research summary returns as the tool result (context-isolated). Sub-agent `tool_call_id` is namespaced as `research:<parent>:<child>`; `stream_mode` stays `"values"`, `chat.py` unchanged.
...
- **hybrid_retriever.py** — `DynamicHybridRetriever`: asymmetric vector + BM25 candidate retrieval. It keeps the vector Top-40 as the primary route, appends up to 20 unique BM25 candidates, then leaves ranking to BGE. Both routes filter by active MySQL chunk ids, so failed or superseded generations cannot be recalled. The BM25 pickle is keyed by a database-derived knowledge fingerprint plus the `preprocess_func` qualname.
